...
days = ["2025-10-23", "2025-10-24", "2025-10-25", "2025-10-26"]
...
# Merge based on rounded timestamps
...
merged['Tin_AHT10'] = merged['Tin_AHT10'] + 0.3407435719249478
merged['RHin_AHT10'] = merged['RHin_AHT10'] + (-2.2783460736622656)
...
# Break line where data missing > 2 mins
...
ax1.set_title('AHT10 vs HMP Comparison from 23 to 26 October 2025 after removing bias')
ax1.grid(True)
...
time_min = merged['Time_rounded'].min()
time_max = merged['Time_rounded'].max()

locator = mdates.HourLocator(interval=2)  # Every 2 hours
...
plt.setp(ax1.xaxis.get_majorticklabels(), rotation=45, ha='center', fontsize=8)
plt.setp(ax2.xaxis.get_majorticklabels(), rotation=45, ha='center', fontsize=8)

plt.tight_layout()
plt.show()

# ================= SAVE MERGED FILE =================
merged.to_csv("AHT10_HMP_Merged_with_Error_23th_29th_oct.csv", index=False)
print("✅ Merged data with errors saved as 'AHT10_HMP_Merged_with_Error_23th_29th_oct.csv'")
